...
router.get('/inquiries', authenticateToken, requireRole('SUPER_ADMIN', 'ADMIN'), asyncHandler(async (request, response) => {
  const status = optionalQueryString(request.query.status, 'status');
  const inquiries = await contentServices.inquiries.list(status as import('../types/index.js').InquiryStatus | undefined);
  response.json({ success: true, data: inquiries });
}));

router.patch('/inquiries/:id/status', authenticateToken, requireRole('SUPER_ADMIN', 'ADMIN'), asyncHandler(async (request, response) => {
