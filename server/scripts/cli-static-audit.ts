...
async function main(): Promise<void> {
  ...
  assert.match(routes, /scenarioSessionFromRequest/);
  ...
  assert.match(service, /ScenarioEngineService/);
  assert.match(service, /executionMode: active \? 'SCENARIO_RUNTIME' : 'RECORDED_STATE'/);
  ...
  assert.match(service, /scenario run <slug>/);
  assert.match(service, /this\.scenarios\.verify/);
  assert.match(service, /this\.scenarios\.remediate/);
  assert.match(service, /this\.scenarios\.reset/);
  ...
  assert.match(terminal, /Unified Recorded-State \+ Scenario CLI/);
  assert.match(terminal, /shell\/provider execution: disabled/);
  assert.match(terminal, /session scenario mutations/);
  ...
  assert.match(docs, /scenario/i);
  assert.match(docs, /external/i);
