    ...
    <div className="space-y-4">
      ...
      <section className="rounded-xl border border-white/10 bg-[#111114] p-4">
        ...
        <div className="grid gap-2 md:grid-cols-2">{operations.scenarioReadiness.map((scenario) => (
          <div key={scenario.id} className="rounded-lg border border-white/10 bg-black/35 p-3"><div className="flex items-center justify-between gap-2"><span className="font-mono text-xs font-bold text-white">{scenario.title}</span><span className="rounded border border-amber-400/20 bg-amber-400/5 px-2 py-1 font-mono text-[8px] uppercase text-amber-300">{scenario.executionAvailable ? 'session runnable' : 'definition only'}</span></div><p className="mt-2 text-xs text-white/50">{scenario.summary}</p>{scenario.observableSignals.length > 0 && <div className="mt-2 flex flex-wrap gap-1">{scenario.observableSignals.map((signal) => <span key={signal} className="rounded border border-white/10 px-1.5 py-0.5 font-mono text-[8px] text-white/35">{signal}</span>)}</div>}</div>
        ))}</div>
